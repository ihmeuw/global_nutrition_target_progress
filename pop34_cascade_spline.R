################################################################################
## DESCRIPTION: Use meta-regression Bayesian prior tool (MR-BRT) and cascading splines to model population of children 2-4 years of age (age_group_id 34)
## INPUTS: 
  #a) Separate population estimates of children in post-neonatal period (age_group_id 4) and in age 1 to 4 years (age_group_id 5), and country-level socio-demographic index (SDI)
  #b) Separate population forecasts of children in post-neonatal period (age_group_id 4) and in age 1 to 4 years (age_group_id 5), and forecasts of country-level SDI
## OUTPUTS: 
  #a) Predicted population of age 2-4 years in training sample (1990-2014), predicted predicted population in testing sample (2015-2021), root-mean squared error (RMSE) ##
  #b) Predicted population forecasts of age 2-4 years from 2021 to 2050 ##
...
#get the combined population for agegroup 5. This will be used as predictor variable in the cascading spline model
population[age_group_id==238, pop238:= population]
population[, pop238:= population[!is.na(pop238)][1], by= c("location_id", "sex_id", "year_id")]

population[age_group_id==4, pop4:= population]
population[, pop4:= population[!is.na(pop4)][1], by= c("location_id", "sex_id", "year_id")]

population[age_group_id==5, pop5:= population]
population[, pop5:= population[!is.na(pop5)][1], by= c("location_id", "sex_id", "year_id")]

population[age_group_id==34, pop34:= population, by= c("location_id", "sex_id", "year_id")]
population[, pop34:= population[!is.na(pop34)][1], by= c("location_id", "sex_id", "year_id")]

population<- population[age_group_id==34,]
population <- merge(population, hierarchy[, .(location_id, super_region_name, region_name)], all.x=T, by = "location_id")
population<- merge(population, sdi[,c("location_id", "year_id", "sdi", "lower_sdi", "upper_sdi")], by=c("location_id", "year_id"), all.x = TRUE)

#se based on UI
# population cannot be put into in logit space
population[, se:= (population - lower)/1.96]

saveRDS(population, file.path(output_dir, "ag34_population2021.RDS"))
pop_34_df <- readRDS(file.path(output_dir, "ag34_population2021.RDS"))
#remove global from this DF
pop_34_df<-pop_34_df[location_id!=1,]

pop_34_df[, proportion:=pop34/pop5]

for(sid in c(1,2)){
  print(cor(population[sex_id==sid]$pop4, population[sex_id==sid]$population))
}
...
fit_pop <- function(model_data,sid) {
  ...
  dat_loc$load_df(
    ...
    col_covs = list("sdi", "pop4","pop5"), col_study_id = "location_id"
  )
  #prediction data
  dat_pred <- MRData()
  dat_pred$load_df(
  ...
  stand_mod <- MRBRT(
    ...
    cov_models = list(
      ...
      LinearCovModel("pop4", use_re = TRUE),
      LinearCovModel("pop5", use_re = FALSE, 
                     use_spline = TRUE,
...
for(sid in c(1,2)){
  pop_standard<- fit_pop(pop_34_df, sid)
  assign(paste0("pop",sid), test_pop(pop_34_df,sid))
  pop_cascade<- fit_pop_cascade(pop_standard, pop_34_df, output_dir1, sid)
}
pop1[,diff:=population-pred_pop]
pop1[,ratio:=population/pred_pop]
summary(pop1$diff)
summary(pop1$ratio)
pop2[,diff:=population-pred_pop]
pop2[,ratio:=population/pred_pop]
summary(pop2$diff)
summary(pop2$ratio)


population<- pop_34_df
...
pdf(file=paste0("FILEPATH", "/mrbrt_plots_pop34_", gsub("-", "", Sys.Date()) ,".pdf"), height = 6.5, width = 10.2)
for(sid in c(1,2)){
  pop_standard<- fit_pop(population, sid)
  pop_cascade<- list(working_dir =  file.path(output_dir1,paste0("cascade_population34", "_", sid)))
  ...
  space <- 0.005
  predictor <- "Age group 5 population"
  exposure <- "population"
  ...
  pop_vec <- seq(min(get(standard)$cov_models[[4]]$spline_knots),max(get(standard)$cov_models[[4]]$spline_knots),by = 500000)
  num_pop <- length(pop_vec)
  sdi_vec <- array(quantile(population[year_id>2004 & year_id<2015 & sex_id== sid,]$sdi))[2:5]
  knot_df <- data.frame(pop5 = rep(pop_vec, 4), sdi_group = rep(1:4, each = num_pop),
                        sdi = rep(sdi_vec, each = num_pop),
                        location_id = 0, sex_id = sid, pop4 = 3000, se = 1)
  ...
  for(sample in c("IS", "OOS")){
    ...
    title_stand_spline <- paste0(sex, " population: pop age 2 to 4 years \n vs. predicted ", measure, " : standard MRBRT")
    title_casc_spline <- paste0(sex, " population: pop age 2 to 4 years \n vs. predicted ", measure, " : cascading spline")
    title_obs_spline <- paste0(sex, " population: pop age 2 to 4 years \n vs. predicted ", measure, " ")
    title_label_stand <- paste0(sex, " population age 2 to 4 years predicted vs. observed ", measure, " in ", time_period, ": standard MRBRT")
    title_label_casc <- paste0(sex, " population age 2 to 4 years predicted vs. observed ", measure, " in ", time_period, ": cascading spline")
    title_label_res_stand<-paste0(sex, " population age 2 to 4 years residuals over time from ", time_period, ": standard MRBRT")
    title_label_res_casc <- paste0(sex, " population age 2 to 4 years residuals over time from ", time_period, ": cascading spline")
...
forecasts_sex_population34<-do.call("rbind",parLapply(cl, c(1,2), function(sid){
  ...
  pop_forecast4_5[age_group_id==4, pop4:= population]
  pop_forecast4_5[, pop4:= na.locf(pop4, na.rm=FALSE), by= c("location_id", "sex_id", "year_id")]
  
  pop_forecast4_5[age_group_id==5, pop5:= population]
  pop_forecast4_5[, pop5:= na.locf(pop5, na.rm=FALSE), by= c("location_id", "sex_id", "year_id")]
  
  
  pop_forecast4_5 <- merge(pop_forecast4_5, hierarchy[, .(location_id, super_region_name, region_name)], all.x=T, by = "location_id")
  pop_forecast4_5<- merge(pop_forecast4_5, sdi_forecast[scenario==0], by=c("location_id", "year_id"))
  #select 1 row per l/y/s
  pop_forecast4_5<- pop_forecast4_5[age_group_id==5,-"population"]
  pop_forecast4_5[,age_group_id:=34]
  pop_forecast4_5[,age_group_name := "2 to 4"]
  ...
  fit_pop_full <- function(model_data,sid) {
    ...
    stand_mod <- MRBRT(
      ...
      cov_models = list(
        LinearCovModel("intercept", use_re = TRUE),
        LinearCovModel("sdi", use_re = FALSE),
        #no spline for pop4 covariate
        LinearCovModel("pop4", use_re = TRUE),
        LinearCovModel("pop5", use_re = FALSE, 
                       use_spline = TRUE,
                       spline_knots = array(seq(0, 1, by = 0.25)), # this tells is to put a spline every 0.25
                       spline_degree = 1L,
                       spline_knots_type = 'frequency',# this specifies to put the knots literally along the domain. 
                       spline_r_linear = TRUE,
                       spline_l_linear = FALSE
        )
      ))
  ...
  population_full <- fit_pop_full(pop_34_df,sid)
  population_cascade_full <- fit_pop_cascade_full(population_full,pop_34_df,output_dir2,sid)
  
  forecast_df <- pop_forecast4_5[sex_id== sid]
  ...
  #add scenario
  preds_loc_forecast[,scenario:=0]
  return(preds_loc_forecast)
}))
forecasts_sex_population34[,location_id:= as.numeric(location_id)]
forecasts_sex_population34 <- merge(forecasts_sex_population34, hierarchy[, .(location_id, location_name)], all.x=T, by = "location_id")
dt<- forecasts_sex_population34[, c("location_id",  "age_group_id", "sex_id", "year_id", "pred_pop_cascade")]
setnames(dt, old= "pred_pop_cascade", new= "value")

#create sex_id 3 for pop_forecast 34
agg_dt<- dt
...
saveRDS(dt, paste0("FILEPATH", "/pop34_locs", gsub("-", "", Sys.Date()) , ".RDS")) 